...
class CustomerPastOrdersVC: BaseVC {
    ...
    var selectedOrderType: OrderStatus = .rejected
    ...
    let segmentedControl : UISegmentedControl = {
        ...
        segmented.insertSegment(withTitle: OrderStatusText.rejected.text, at: 0, animated: true)
        segmented.insertSegment(withTitle: OrderStatusText.canceled.text, at: 1, animated: true)
    ...
    private func customizeNavBar(){
        setNavBarTitle("My Past Orders".getLocalizedString())
    }
...
// TABLE VIEW
extension CustomerPastOrdersVC: UITableViewDelegate, UITableViewDataSource{
    ...
    func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        case .rejected:
            if let orders = groupedOrders[.rejected]{
                if orders.isEmpty {
        ...
        case .canceled:
            if let orders = groupedOrders[.canceled]{
                if orders.isEmpty {
                    return emptyOrdersTableCellHeight
                }else{
                    return ordersTableCellHeight
...
// FIREBASE OPERATIONS
extension CustomerPastOrdersVC {
